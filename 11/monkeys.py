        ...
        for m in monkey_index.keys():
            for i in monkey_index[m]['items']:
                ...
                if monkey_index[m]['operation']['operator'] == '+':
                    worry_level += monkey_index[m]['operation']['modifier'] if monkey_index[m]['operation']['modifier'] != 'x' else worry_level
                else:
                    worry_level *= monkey_index[m]['operation']['modifier'] if monkey_index[m]['operation']['modifier'] != 'x' else worry_level
                if part == 1:
                ...
                if worry_level % monkey_index[m]['test']['factor'] == 0:
                    monkey_index[monkey_index[m]['test']['true']]['items'].append(worry_level)
                else:
                    monkey_index[monkey_index[m]['test']['false']]['items'].append(worry_level)
                result[m] += 1
