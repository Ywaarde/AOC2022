...
def tree_vis(input, x, y):
    ...
    hor_left = [
            tree
            for i, tree in enumerate(input[y])
            if i in range(x)
        ]
    ...
    ver_up = [
        tree
        for i, tree in enumerate(vert_trees)
        if i in range(y)
    ]
    ver_down = [
        tree
        for i, tree in enumerate(vert_trees)
        if i in range(y+1, len(vert_trees))
        ]
    # print(hor_left, '/', input[y][x], '/', hor_right)

    if (shorter_trees(hor_left, input[y][x]) 
