...
def priority(input):
    ...
    for sack in input:
        ...
        second = sack[mid:]
        for fi in first:
            match = next((si for si in second if si == fi), "")
            if match:
                result += match
                break
...
def badges(input):
    ...
    for i in range(groups):
        ...
        for first in input[i * 3]:
            match = next((second for second in input[i * 3 + 1] if second == first), "")
            if match not in first_match:
                first_match += match
        for third in input[i * 3 + 2]:
            match = next((found for found in first_match if found == third), "")
            if match:
                result += match
                break
